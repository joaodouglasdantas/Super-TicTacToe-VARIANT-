...
import type { CardId, GameConfig, Move, Path, Player } from '../engine';
...
export type P2PMessage =
  ...
  | { t: 'accept' }
  // spec CARTAS: card/path2/cellIndex só presentes quando a jogada é uma
  // carta (Move.card definido) — mesmo formato que o histórico usa internamente.
  | { t: 'move'; seq: number; path: Path; card?: CardId; path2?: Path; cellIndex?: number }
  | {
      t: 'sync';
      config: GameConfig;
      hostSymbol: Player;
